...
- Every site gets a free JSON backend: shared state with atomic ops and append-only collections. Reads are public and Origin-gated. On the shared host any page can read and write them with fetch (PATCH /state with {ops:[...]} or POST /collections/{name}, always with credentials:"include" and header X-SH-CSRF: 1). Load <script src="https://simple-host.app/auth.js" defer></script>, put <div id="sh-auth"></div> near the form, call SH.mount('#sh-auth'), and await SH.requireSignIn() before saving: on the shared host it resolves at once, on a custom domain it signs the visitor in (Google or an emailed code) so saves are per-person. Agents write with X-API-Key, including on the shared host.
