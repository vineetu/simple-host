...
- Only external resources allowed: Google Fonts, and optionally the simple-host comments widget (<script src="https://simple-host.app/comments.js" defer></script> with <section id="sh-comments"></section>). If the page embeds comments.js, the hosted widget handles sign-in; do not invent anonymous fetch writes.
...
- GitHub Copilot CLI: its ~/.copilot folder is sandboxed, so unzipping into it fails. Use: copilot skill add https://simple-host.app/skills/<skill-name>/SKILL.md (once per skill), or npx skills add vineetu/simple-host -g -a github-copilot -s <skill-name> --copy -y.
