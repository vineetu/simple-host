...
- Every site gets a free JSON backend: shared state with atomic ops and append-only collections. Reads are public and Origin-gated. Pages can only save data (state/collections) when the site has its own custom domain. On a custom domain include <script src="https://simple-host.app/auth.js" defer></script>, SH.mount('#sh-auth'), and await SH.requireSignIn() before SH.state.patch(...) / SH.collection(name).append(...). Sign-in is Google or an emailed code. On the shared host build the page without a save feature (or with a clear "connect a domain to enable saving" note). Agents write with X-API-Key, including on the shared host.
