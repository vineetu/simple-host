...
- Claude APP (claude.ai on web/desktop/mobile — NOT Claude Code): there is no terminal. Settings → Skills → "+ Create skill", then upload each ZIP separately: simple-host.app/skills/website-deploy.zip, /skills/website-deploy-builder.zip, /skills/connect-domain.zip. Needs a paid plan with Code execution switched on. If someone says "Claude" always check which of the two they mean.
...
- ChatGPT in a browser, Gemini, browser Copilot: cannot install anything. Use the copy-a-prompt flow on simple-host.app/install.html — paste the prompt there, paste the JSON it returns back at simple-host.app, hit Publish.
